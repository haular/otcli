...
live at the top of ``otcli/bootstrap.py``. Side-effects on import break
sandboxes, read-only containers, and tests that rely on a clean tmp
``$HOME``.
"""
...
def test_importing_otcli_does_not_touch_home(monkeypatch, tmp_path: Path) -> None:
    ...
    importlib.import_module('otcli.bootstrap')
    importlib.import_module('otcli.cli.app')

    config_dir = fake_home / '.otcli_config'
    assert not config_dir.exists(), (
        f'Importing otcli created {config_dir} \u2014 this is a regression. ' f'No I/O is allowed at import time.'
    )
